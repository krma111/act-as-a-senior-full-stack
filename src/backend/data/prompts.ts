...
import { creatorSlug, slugify } from "@/shared/constants/slugs";
import type { Category, Profile, Prompt, SiteSettings } from "@/shared/types";
...
async function withRetry<T>(label: string, operation: () => Promise<T>, fallback: T, attempts = 2): Promise<T> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      lastError = error;
      console.error(`[data] ${label} failed on attempt ${attempt}`, error);
    }
  }

  console.error(`[data] ${label} returned fallback after retries`, lastError);
  return fallback;
}
...
export async function getSiteSettings(): Promise<SiteSettings> {
  return withRetry("getSiteSettings", async () => {
    ...
    return {
      ...defaultSiteSettings,
      website_name: data?.website_name ?? defaultSiteSettings.website_name,
      logo_text: data?.logo_text ?? defaultSiteSettings.logo_text,
      hero_headline: data?.hero_headline ?? defaultSiteSettings.hero_headline,
      hero_subheadline: data?.hero_subheadline ?? defaultSiteSettings.hero_subheadline,
      footer_text: data?.footer_text ?? defaultSiteSettings.footer_text,
      cta_text: data?.cta_text ?? defaultSiteSettings.cta_text,
      empty_state_title: data?.empty_state_title ?? defaultSiteSettings.empty_state_title,
      empty_state_message: data?.empty_state_message ?? defaultSiteSettings.empty_state_message,
...
export async function getCategories(): Promise<Category[]> {
  return withRetry("getCategories", async () => {
...
export async function getPrompts(options?: {
  ...
  return withRetry("getPrompts", async () => {
    if (!hasSupabaseEnv) return [];
    ...
    if (error) {
      console.error("[data] getPrompts query error", error);
      return [];
    }
...
export async function getPrompt(idOrSlug: string) {
  return withRetry("getPrompt", async () => {
...
export async function getPromptsByCreator(username: string) {
  return withRetry("getPromptsByCreator", async () => {
  ...
  }, { creator: null as Profile | null, prompts: [] as Prompt[] });
}

export async function getCreatorLeaderboard() {
  return withRetry("getCreatorLeaderboard", async () => {
    if (!hasSupabaseEnv) return [];
