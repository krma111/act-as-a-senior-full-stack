...
export async function PromptGrid({ prompts, empty }: { prompts: Prompt[]; empty: string }) {
  ...
  if (!prompts.length) {
    return (
      <div className="card-surface min-w-0 rounded-[32px] border-dashed p-8 text-center text-slate-400 sm:p-10">
  ...
  return (
    <div className="grid-cascade grid min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {prompts.map((prompt) => (
