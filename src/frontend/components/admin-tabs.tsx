...
export function AdminTabs({ active }: { active: string }) {
  return (
    <motion.nav
      className="premium-scrollbar mb-8 flex w-full max-w-full gap-2 overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.055] p-2 shadow-[0_22px_70px_rgba(0,0,0,0.32)] backdrop-blur-xl"
    ...
    >
      {tabs.map((tab) => (
        <motion.div key={tab.href} className="shrink-0" whileHover={{ y: -1, scale: 1.02 }} whileTap={{ scale: 0.97 }}>
          <Link
            href={tab.href}
            className={`relative block whitespace-nowrap rounded-xl px-4 py-2 text-sm font-bold transition ${
