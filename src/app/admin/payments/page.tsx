    ...
    <MotionMain className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {filters.map((filter) => (
            <Link key={filter} href={filter === "all" ? "/admin/payments" : `/admin/payments?status=${filter}`} className={(params.status ?? "all") === filter ? "btn-primary" : "btn-ghost"}>
              {filter.replace("_", " ")}
            </Link>
          ))}
        </div>
      ...
      {!orderResult.orders.length ? (
        ...
        <MotionSection className="grid gap-5">
          {orderResult.orders.map((order) => (
            <article key={order.id} className="card-surface rounded-[32px] p-5 sm:p-6">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  ...
                  <h2 className="mt-3 text-xl font-bold text-white">{order.prompt_pack_title}</h2>
                  <p className="mt-1 text-sm text-slate-400">{order.buyer_email} - {formatDate(order.created_at)}</p>
                ...
                <div className="grid gap-3 xl:min-w-[360px]">
                  <form action={updateMvpOrderStatus} className="grid gap-2 sm:grid-cols-[1fr_auto]">
                    ...
                    <AdminSubmitButton className="btn-primary justify-center" pendingText="Updating...">Update</AdminSubmitButton>
                  </form>
              ...
              <p className="mt-4 rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-xs leading-6 text-slate-400">
